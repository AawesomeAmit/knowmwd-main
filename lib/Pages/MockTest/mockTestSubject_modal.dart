...
import 'mockTestSubject_controller.dart';

class MockTestSubjectModal {
  MockTestSubjectController controller = Get.put(MockTestSubjectController());
  App app = App();
  UserData user = UserData();
  QuizBankTopicData quizBankTopicData=QuizBankTopicData();
  // QuizBankListModal quizBankList=QuizBankListModal();

  getQuizList(context) async{
    // controller.updateShowNoData=false;
    // ProgressDialogue().show(context, loadingText: 'Please wait...');
    var body={
      'userId': user.getUserId.toString(),
    };
    var data=await app.api('otQuizBankList', body, context);
    ...
    if(data['status']==0){
      ...
      if(data['responseCode']==0){
        ...
        controller.updateQuizBankList(data['responseValue']);
  ...
  onPressSubject(context, int id){
    ...
    app.navigate(context, QuizBankTopicView(id.toString()));
  }
}
